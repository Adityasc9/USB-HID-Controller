...
//! from VDDA(3V) to 0V.  The RED LED on PF1 is used to indicate gamepad
...
//*****************************************************************************
//
// The HID gamepad polled ADC data for the X/Y/Z coordinates. X = Joystick X, Y = Joystick Y, Z = Potentiometer
//
//*****************************************************************************
...
main(void)
{
    ...
    UARTprintf("\033[2JROCKET LEAGUE GAMEPAD\n"); // clear screen then print
    UARTprintf("---------------------------------\n\n");
    ...
    while(1)
    {
        ...
        if(g_iGamepadState == eStateIdle)
        {
            ...
            // check buttons
            if(ui8Buttons & BUTTON1_MASK)
            {
                sReport.ui8Buttons |= BUTTON1_MASK;
            }

            if(ui8Buttons & BUTTON2_MASK)
            {
                sReport.ui8Buttons |= BUTTON2_MASK;
            }

            if(ui8Buttons & BUTTON3_MASK)
            {
                sReport.ui8Buttons |= BUTTON3_MASK;
            }

            if(ui8Buttons & BUTTON4_MASK)
            {
                sReport.ui8Buttons |= BUTTON4_MASK;
            }

            if(ui8Buttons & JOYSTICK_MASK)
            {
                sReport.ui8Buttons |= JOYSTICK_MASK;
            }
            ...
            if(ADCIntStatus(ADC0_BASE, 0, false) != 0)
            {
                ...
                if (pot < 2048) {
                    ...
                    sReport.i8RT = 0;       // turn off
                } else if (pot > 2048) {
                    sReport.i8LT = 0;       // turn off
